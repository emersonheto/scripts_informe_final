    ...
    BEGIN TRY
		...
		WITH datos_base AS (
				SELECT 
						...
						VERSION_PLAN,PROGRAM_CODE,DEPT_CODE,ASIGNATURA,ESTADO_ASIGNATURA,PORCENT_INASISTENCIA,to_number(NVL(GRDE_CODE,''0'')) AS NOTA,
		...
		T_NOTAS AS (
				SELECT PIDM,DNI,STUDYPATH_BLOQUE,STUDYPATH_STATUS_DESC,NOMBRE,NOMBRE_CURSO,VERSION_PLAN,PROGRAM_CODE,DEPT_CODE,ASIGNATURA,ESTADO_ASIGNATURA,PORCENT_INASISTENCIA, NOTA
				FROM datos_base
        ...
        ,T_APROBADOS AS (
            ...
            ) B ON B.TERM_CODE_EFF=A.VERSION_PLAN AND B.PROGRAM=A.PROGRAM_CODE AND B.MODALIDAD=A.DEPT_CODE

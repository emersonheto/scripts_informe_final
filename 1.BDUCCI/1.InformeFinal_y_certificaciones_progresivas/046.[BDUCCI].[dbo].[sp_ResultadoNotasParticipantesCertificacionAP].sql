...
CREATE PROCEDURE [dbo].[sp_ResultadoNotasParticipantesCertificacionAP]
(
	 @XmlStudents XML,
	 @AreaCert VARCHAR(20)
)
...
BEGIN
	BEGIN TRY
       ...
       SELECT CODIGO,APELLIDOS_NOMBRES,CURSO,NOTA,PROMEDIO,TIPO_ALUMNO,ESTADO_ACADEMICO,ESTADO_CAPP
		   ...
		   WITH T_NOTAS AS (
			  SELECT PIDM,DNI,STUDYPATH_BLOQUE,STUDYPATH_STATUS_DESC,NOMBRE,NRC||'''' - ''''||NOMBRE_CURSO AS NOMBRE_CURSO,
						 NVL(STYP_DESC,'''' '''') AS TIPO_ALUMNO,VERSION_PLAN,PROGRAM_CODE,DEPT_CODE,A.ASIGNATURA,ESTADO_ASIGNATURA,
			   ...
			   SELECT C.DNI AS CODIGO,C.NOMBRE AS APELLIDOS_NOMBRES,C.NOMBRE_CURSO AS CURSO,C.NOTA,D.PROMEDIO,TIPO_ALUMNO,
					  D.ESTADO_ACADEMICO,

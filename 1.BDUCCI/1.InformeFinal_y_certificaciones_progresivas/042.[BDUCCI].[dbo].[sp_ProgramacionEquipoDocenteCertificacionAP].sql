...
BEGIN
	BEGIN TRY
       ...
       SELECT CICLO,NOMBRE_CURSO,NOMBRE_DOCENTE,HORAS_LECTIVAS,FECHA_INICIO,FECHA_FIN
		   FROM OPENQUERY ('+@BDOracle+',''
			  SELECT CICLO,NOMBRE_CURSO,NOMBRE_DOCENTE,HT AS HORAS_LECTIVAS,MIN(FECHA_INICIO) AS FECHA_INICIO,MAX(FECHA_FIN) AS FECHA_FIN
				  FROM (
					 SELECT DISTINCT SUBSTR(A.AREA_DESC,9,3) AS CICLO,A.NOMBRE_CURSO,A.NOMBRE_DOCENTE,B.HT,C.SSRMEET_START_DATE AS FECHA_INICIO,C.SSRMEET_END_DATE AS FECHA_FIN
